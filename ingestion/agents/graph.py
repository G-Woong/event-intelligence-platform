...
from ingestion.agents.state import CrawlingAgentState
from ingestion.core.retry_policy import STRATEGY_SEQUENCE

logger = logging.getLogger("ingestion.agents.graph")
...
_SELENIUM_STRATEGIES = {"selenium_rendered_dom"}
...
def _fetch_with_strategy(url: str, strategy: str, run_id: str, source_id: str, uh: str) -> tuple[Optional[str], Optional[Path], Optional[Path]]:
    ...
    """
    from ingestion.core.artifact_store import get_screenshot_path, save_dom_snapshot, build_dom_snapshot_dict
    ...
    elif strategy in _SELENIUM_STRATEGIES:
        from ingestion.fetch_strategies.selenium_strategy import SeleniumRenderStrategy, SeleniumFetchResult
        result = SeleniumRenderStrategy().fetch(url)
        if result.status == "NOT_READY":
            logger.warning("selenium NOT_READY for %s — chromedriver absent", source_id)
...
def _node_fetch_entry_url(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.core.artifact_store import save_raw_html
    from ingestion.sources._registry import get_source_instance

    src = get_source_instance(source_id)
    try:
        custom_html = src.fetch_entry_html(url) if src else None
    except Exception as _exc:
        logger.warning("[%s] fetch_entry_html error: %s", source_id, _exc)
        custom_html = None
    if custom_html is not None:
        rp = save_raw_html(run_id, source_id, uh, "entry_custom", custom_html)
        return {"raw_html": custom_html, "current_url": url, "raw_html_path": str(rp)}
...
def _node_extract_candidate_urls(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.sources._registry import get_source_instance
    from ingestion.core.artifact_store import url_hash as make_uh
...
def _node_fetch_target_page(state: CrawlingAgentState) -> dict:
    ...
    src = get_source_instance(source_id)
    try:
        custom_html = src.fetch_page_html(url, strategy) if src else None
    except Exception as _exc:
        logger.warning("[%s] fetch_page_html error: %s", source_id, _exc)
        custom_html = None
    if custom_html is not None:
        rp = save_raw_html(run_id, source_id, uh, f"custom_{strategy}", custom_html)
        return {"raw_html": custom_html, "current_url": url, "raw_html_path": str(rp)}
...
def _node_extract_content(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.core.artifact_store import save_raw_html, save_extracted_text
    ...
    if need_refetch:
        ...
        from ingestion.core.artifact_store import save_dom_snapshot, get_screenshot_path, build_dom_snapshot_dict
    ...
    # source-specific extract() hook — API/custom path, skips blocker check and HTML cascade
    from ingestion.sources._registry import get_source_instance as _get_src
    _src = _get_src(source_id)
    if _src:
        try:
            custom_result = _src.extract(html, url, strategy)
        except Exception as _exc:
            logger.warning("[%s] src.extract() error: %s", source_id, _exc)
            custom_result = None
        if custom_result is not None:
            custom_result.setdefault("url", url)
            custom_result.setdefault("source_id", source_id)
            raw_payload = custom_result.pop("raw_payload", None)
            payload_format = custom_result.pop("payload_format", "json")
            rp_path: Optional[str] = None
            ep_path: Optional[str] = None
            if raw_payload is not None:
                from ingestion.core.artifact_store import save_raw_payload, save_extracted_payload
                _rp = save_raw_payload(run_id, source_id, uh, payload_format, raw_payload)
                _ep = save_extracted_payload(run_id, source_id, uh, custom_result)
                rp_path = str(_rp)
                ep_path = str(_ep)
            txt_path = save_extracted_text(run_id, source_id, uh, strategy, custom_result)
            tried = list(state["strategies_tried"]) + [strategy]
            return {
                "extraction_result": custom_result,
                "strategies_tried": tried,
                "extracted_text_path": str(txt_path),
                "raw_payload_path": rp_path,
                "extracted_payload_path": ep_path,
            }
...
def _node_score_quality(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.core.quality_score import build_metrics_from_extraction, compute_quality_score, determine_quality_status
    from ingestion.core.error_taxonomy import classify_content_blocker
...
def _node_error_analysis(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.core.error_taxonomy import ErrorType, BLOCKED_ERRORS, RETRYABLE_ERRORS
...
def _node_extract_event_candidates(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.agents.llm_judge import create_judge_client
...
def _node_write_source_report(state: CrawlingAgentState) -> dict:
    ...
    from ingestion.sources._registry import get_source_instance as _get_src_report

    final_status = state["quality_status"]
    _src_r = _get_src_report(source_id)
    if _src_r and final_status in ("FAILED", "BLOCKED"):
        try:
            _fallback = _src_r.fallback_status()
            if _fallback:
                final_status = _fallback
        except Exception:
            pass
    ...
    row = {
        ...
        "artifact_paths": {
            ...
            "dom_snapshot": state.get("dom_snapshot_path"),
            "extracted_text": state.get("extracted_text_path"),
            "raw_payload": state.get("raw_payload_path"),
            "extracted_payload": state.get("extracted_payload_path"),
        },
