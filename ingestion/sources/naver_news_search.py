...
class NaverNewsSearchSource(SourceCrawler):
    def build_search_query(self, keywords: list[str] | None = None) -> str:
        return "naver news 최신"

    def get_entry_url(self) -> str:
        return "https://openapi.naver.com/v1/search/news"
    ...
    def precheck_status(self) -> Optional[dict]:
        ...
        if not client_id or not client_secret:
            return {
                "status": "NEEDS_API",
                "reason": "NAVER_CLIENT_ID / NAVER_CLIENT_SECRET not set in .env (Naver Search API 등록 필요: https://developers.naver.com)",
            }
        return None
