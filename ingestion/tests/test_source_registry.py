...
from ingestion.core.source_registry import SourceRegistry, SourceSpec, load_registry
...
def test_registry_has_30_real_sources():
    ...
    real_sources = [
        ...
        if s.id != "_dummy" and s.extra.get("implemented", True)
    ]
...
def test_registry_extended_candidates_exist():
    registry = load_registry(_CONFIGS_DIR)
    extended = [s for s in registry.all() if s.extra.get("implemented", True) is False]
    assert len(extended) >= 20, f"Expected >=20 phase-4 candidates, got {len(extended)}"


def test_all_sources_have_layer():
    registry = load_registry(_CONFIGS_DIR)
    for s in registry.all():
        assert s.layer != "", f"Source '{s.id}' has empty layer field"


def test_source_spec_layer_field():
    registry = load_registry(_CONFIGS_DIR)
    bbc = registry.get("bbc")
    assert bbc is not None
    assert bbc.layer == "document_discovery"
    finnhub = registry.get("finnhub")
    assert finnhub is not None
    assert finnhub.layer == "market_signal"
