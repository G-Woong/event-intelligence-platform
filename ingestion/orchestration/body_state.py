...
# RSS/Atom 피드가 본문 대신 싣는 "발췌 + 전문 링크" 꼬리표. 길이가 임계를 넘어도
# 전문이 아니므로 present로 승격하지 않는다(긍정편향 금지: excerpt ≠ full body).
_EXCERPT_MARKERS = (
    "read the full story", "read full story", "read the rest", "read more at",
    "continue reading", "read full article", "appeared first on",
    "the post appeared", "[…]", "[…]", "click here to read",
)


def _looks_truncated(body: str) -> bool:
    """본문이 피드 발췌(전문 아님)로 보이는지. 끝부분 연속 마커를 검사한다."""
    tail = body[-200:].lower()
    return any(m in tail for m in _EXCERPT_MARKERS)
...
def assess_body_state(
    *,
    ...
    # 발췌(전문 링크 꼬리표)는 길이가 임계를 넘어도 full body가 아니다 → snippet_only로 강등.
    if body and _looks_truncated(body):
        return BodyExtractionState(
            body_missing=True, body_source="body_excerpt", body_length=blen,
            extraction_status="snippet_only", numeric_payload_exempt=False,
            snippet_only=True, partial=False, reason="excerpt_with_full_story_link",
        )
