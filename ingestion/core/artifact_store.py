...
logger = logging.getLogger("ingestion.core.artifact_store")
...
def save_raw_payload(run_id: str, source_id: str, h: str, fmt: str, payload: str) -> Path:
    ext = fmt if fmt in ("json", "xml", "csv") else "dat"
    path = _OUTPUTS_DIR / "raw_payload" / source_id / f"{run_id}_{h}_{fmt}.{ext}"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(payload, encoding="utf-8")
    logger.debug("raw_payload saved: %s", path)
    return path


def save_extracted_payload(run_id: str, source_id: str, h: str, fields: dict) -> Path:
    path = _OUTPUTS_DIR / "extracted_payload" / source_id / f"{run_id}_{h}.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(fields, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    logger.debug("extracted_payload saved: %s", path)
    return path


def save_raw_signal(run_id: str, source_id: str, h: str, payload: str) -> Path:
    """Save structured signal payload (JSON text) to raw_signal/."""
    path = _OUTPUTS_DIR / "raw_signal" / source_id / f"{run_id}_{h}.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(payload, encoding="utf-8")
    logger.debug("raw_signal saved: %s", path)
    return path


def save_rendered_dom(run_id: str, source_id: str, h: str, html: str) -> Path:
    """Save Playwright-rendered HTML to rendered_dom/."""
    path = _OUTPUTS_DIR / "rendered_dom" / source_id / f"{run_id}_{h}.html"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(html[:200_000], encoding="utf-8")
    logger.debug("rendered_dom saved: %s", path)
    return path
