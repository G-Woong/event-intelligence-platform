# 12 — 오케스트레이션 구축 개요 및 사용자 결정 사항

> ⚠️ **상태 정정 (2026-06-19, 이관):** 본 문서는 `docs/Orchestration_Construction/`(폐지)에서 `_CANONICAL/`로 이관됨. **아래 "코드를 적용하지 않는다 / 구현은 다음 턴" 선언은 작성 시점(2026-06-12) 기준이며 현재는 stale** — 오케스트레이션 엔진은 **구현 완료**됨(현재 사실의 단일출처: `_CANONICAL/01·02`). 본 문서는 §7.1 Phase 정의·사용자 결정 사항의 설계 근거로 보존한다.
