...
| ap_news | AP News | Google News RSS 프록시 (canonical=apnews.com) | LIVE_SUCCESS (100건) | **CORE_READY** |
...
| caution (조건부 — 정비 후 투입) | 8 | **gdelt(429 ×2 실측 — 02 cooldown 기록 경로로 보호)**, ~~ap_news~~ → **복구 완료(03: Google News RSS 프록시, LIVE_SUCCESS 100건, 2026-06-13)**, ~~newsapi~~ → **복구 완료(04: /v2/everything 전환, LIVE_SUCCESS relevance high, 2026-06-13)**, signal_bz, loword/google_trending_now/dcinside/eu_press_corner(selector 보강) |
...
### ap_news 복구 후속 (2026-06-13 anomaly closure)

- **active endpoint**: `https://news.google.com/rss/search` (params: q=site:apnews.com, hl/gl/ceid). LIVE_SUCCESS 100건.
- **retired endpoint(폐기)**: `https://apnews.com/hub/ap-top-news?format=feed&type=rss` — 파라미터 무시 후 홈 HTML 200. 더 이상 active 아님(테스트로 고정).
- **rejected 후보**: rsshub.app — Cloudflare 403 challenge. 우회 금지, active path 아님.
- **redirect 정규화 (CLOSED, 2026-06-13 2차)**: item link는 news.google.com redirect URL. HTTP HEAD/GET은 news.google.com에 머물지만(JS 디코드), **Playwright headless 1-hop**(`url_resolver.resolve_via_browser`)이 JS navigation을 따라가 원본 apnews.com 기사 URL에 도달 — live 3/3 정규화. `extract_sample_items(resolve_canonical=True, canonical_via_browser=True)`로 sample에 canonical_url(apnews.com) 부착. 일반 redirect는 HTTP resolve, 동의/CAPTCHA는 우회 않고 원본 반환.
- **evidence_level (DISCOVERY_PROXY_NOTE)**: canonical AP URL 확보로 AP source identity 회복(tier1 유지). discovery 경로가 Google News RSS 프록시라는 점만 표기 — **WARNING/CAVEAT 아님**.
- **newsapi**: `/v2/everything`(country 미포함, 기본 q) LIVE_SUCCESS relevance high — active WARNING 없음.
- 테스트: 전체 **544 passed**, secret scan PASS(1043 files). live: Playwright 1-hop 정규화 3/3.

## 08/09 라운드 갱신 (API partial/no 소스 seed·signal 종결, 2026-06-13)

직전 라운드 connectivity(LIVE_SUCCESS)는 유지하되, **sample→candidate/numeric_signal 정제 readiness**를 종결했다 (`run_api_partial_sources_audit.py`, JSONL `api_partial_sources_e2e_audit_20260613_092502.jsonl`).

| source_id | 직전 gap | 적용 fix | 이번 status |
|---|---|---|---|
| federal_register | url/date 부재(fields[]=title) | fields[]→5필드 list | seed_ready **yes**, body_extracted 1 |
| igdb | url 미요청/epoch 미변환/$root 미매핑 | apicalypse url + _SAMPLE_PATHS $root + epoch→ISO | seed_ready **yes** |
| culture_info | XML 필드 매핑 부재 | _XML_FIELD_NAMES(title/startDate/realmName) | seed_ready **yes** (key=CULTURE_INFO_KEY alias) |
| hacker_news | id 배열만(detail 미설계) | detail 2차 호출(/v0/item/{id}.json) + extracted fallback | seed_ready **yes**, body_extracted 1 |
| bok_ecos / eia / its | _SAMPLE_PATHS 부재 | 매핑 추가 + numeric_signal 분류 | **signal_ready** |
| finnhub (시장 수치) | seed 5필드로 영원히 no | NUMERIC_SIGNAL_SOURCES + flat quote signal | **signal_ready** |

공식 `run_primary_seed_live_audit --sources` 8 subset 재검증: 전부 LIVE_SUCCESS, news형 4=yes / 수치형 4=signal_ready, seed_ready no(signal 제외)=0.

## 갱신 (docs/10 최종 closing, 2026-06-13)

- **gdelt**: DEFERRED(live) → **PASS(collected)**. gate 통과 live 1회 LIVE_SUCCESS items=3 + body 1건. 빠른 연속 호출만 soft-429 → min_interval 60s로 운영(근거: rate_limit_evidence.md).
- **google_trends_explore**: **RATE_LIMITED_CONFIRMED**(외부 provider 429, robot.png rendered DOM). optional enrichment로 유지. NOT_READY_EXTERNAL_RATE_LIMIT.
- **enrichment live audit 실측**(enrichment_live_audit_20260613_095628): serper/tavily/exa/naver×2/gnews/newsapi/guardian/nyt/sec_edgar/youtube/tmdb live 33회 전부 collected, high relevance 27. 본문 추출 3건(guardian/gnews/newsapi).
- **runner orchestration readiness**: 12개 수집 스크립트 전부 agent_ready(runner_orchestration_readiness_*).

## Google Trends fallback (PHASE 2, 2026-06-13)

- **google_trends_explore**: RATE_LIMITED_CONFIRMED 유지(외부 429). 단, optional_enrichment이며 **fallback chain으로 대체 경로 확보** → event queue 비차단.
- **google_trending_now**: PASS. 추가로 **공개 RSS export 경로 확인**(google_trends_trending_now_export, EXPORT_AVAILABLE) — Playwright 외 안정 대체 seed.
- **fallback 실측**: trend_fallback_enrichment_audit_20260613_102354 — collected fallback source 5, aggregate related_candidate 19, body 1건. 신규 runner `run_trend_fallback_enrichment_audit.py`(agent_ready).
