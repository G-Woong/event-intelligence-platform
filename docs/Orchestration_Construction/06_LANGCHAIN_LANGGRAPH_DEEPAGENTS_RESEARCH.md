...
```
Layer 1. Deterministic ingestion orchestration   ← 이번 설계 본체 (Phase A~E, 설치 0)
  ...
  → 프레임워크 없음. 순수 Python. LLM 없음(quality_judge만 선택적).
...
Layer 3. Future high-level agent layer            ← MVP 이후, 지금 설치/도입 안 함
  사건 확장 조사 / 소스 발견 / 증거 탐색 / 팀 에이전트 리뷰 / 관리자 리서치
```
...
| **Deep Agents** | **사건 확장 조사·증거 탐색·리서치(우선 후보)** | ❌ | **MVP 이후 Layer 3 1순위** | 가상 FS/Shell 고위험, 비결정성, 비용 | `INSTALL_CANDIDATE_REQUIRES_USER_APPROVAL` (지금 금지) |
...
**판단 근거(공식·코드 기준)**:
- 수집 본체(Layer 1)는 **deterministic이어야** 비용·비결정성·디버깅을 통제한다. 어떤 에이전트 프레임워크도 Layer 1에 넣지 않는다.
- 사건 처리(Layer 2)는 **이미 LangGraph 0.2.76으로 동작**한다. 새 프레임워크로 갈아끼우는 것은 회귀 위험 대비 이득이 없다.
- Layer 3은 "사람이 묻는 심층 리서치/확장 조사" 같은 **MVP 이후 부가가치**다. 그때 **Deep Agents를 1순위로 평가**하되, 설치는 사용자 승인 후. CrewAI는 팀 리뷰 비교용, MS Agent Framework는 장기 후보로만 문서화.
