...
**판정: PARTIAL_MIXED_PENDING_AND_BLOCKERS**. 본문/URL 복원력 관점에서 가장 중요한 점은 **dcinside가 full body를 의도적으로 추출하지 않는다**는 보수적 결정이다.

- **dcinside — list 메타데이터 preview only, full article body 미수집(이 때문에 DEGRADED 강등)**. `dcinside_strategy.py`는 robots 허용 갤러리의 **list HTML에서 제목+URL+ISO time anchor만** 파싱한다(live 실증: stockus 갤러리 → 30 항목 전부 세 필드 보유). 개별 게시글 본문 페이지로 들어가 full text를 긁지 않는다. 이는 04 cascade의 "끝까지 본문을 못 뽑아도 단서(제목·URL·시각)는 버리지 않는다" 원칙의 **역방향 적용**이다 — 커뮤니티 저작물에 대해서는 단서만으로 충분하며, full body 추출은 저작권상 보수적으로 생략한다. body가 비어 있다는 바로 그 사실(LIST_PREVIEW_ONLY_NO_BODY) 때문에 최종 등급은 clean READY가 아니라 **PRODUCTION_READY_WITH_PUBLIC_PREVIEW_ONLY(=production_state DEGRADED)**로 정직히 강등됐다(cnbc/nyt가 snippet/preview-only로 강등된 선례와 동일한 본문-부재 기준). body_extraction cascade(trafilatura/readability/dom)를 이 소스에는 태우지 않는다. extraction_status는 사실상 list_preview로 고정되고, snippet/preview를 body_present로 계수하지 않는 기존 규칙과 정합한다.
- **URL 복원력 측면**: dcinside list anchor는 갤러리 게시글의 직접 URL이라 url_resolver 단계가 불필요하다(Google News류의 URL 은닉 없음). Cloudflare 챌린지/CAPTCHA가 본문/list 어느 단계에서든 감지되면 cascade로 더 내려가지 않고 `*_BLOCKED_NO_BYPASS`로 즉시 중단한다(렌더 강행·우회 금지).
- **gdelt / google_trends_explore**: 둘 다 본문 추출 대상이 아니다(gdelt=structured/article reference 신호, g_trends=trending 신호). gdelt는 429로 pending_resume, g_trends는 requires_official_api_or_contract blocker이므로 이번 cascade 변경 범위 밖이다.

## Phase G-3 — Final Source Closure

**판정: PARTIAL_WITH_VERIFIED_HARD_BLOCKERS**. URL/본문 복원력 관점에서 이번 단계의 핵심은 **두 소스의 합성/죽은 URL을 실제 url로 해소**하고, dcinside 본문 부재를 정직히 재확인한 것이다.

- **culture_info — detail2 실 url 해소(NO_STABLE_URL 종료)**. 기존 adapter는 죽은 `culture.go.kr/wantU/detailView.do?seq=` shell(909바이트, 내용 없음)을 합성했다. G-3 fix: data.go.kr `period2`(list)→`detail2`(seq별) 호출로 **실제 외부 전시 url** 확보(예: `sma.sbculture.or.kr/.../current.do`, `gmoma.ggcf.kr/exhibitions/191`, `buyeo.museum.go.kr/.../view.do`) + startDate 시간 anchor + seq stable id. 라이브 5건. 적대 리뷰 흡수(HIGH-3): 전시 고유 url 없으면 **placeUrl(장소 홈페이지) 폴백을 제거하고 스킵** — 무관 venue url을 전시 url로 둔갑시키지 않는다.
- **product_hunt — 합성 slug url 제거**. name 기반 합성 slug(NO_STABLE_URL/NO_TIMESTAMP) 대신 GraphQL `url`로 utm 추적 쿼리 제거한 canonical url(예: `https://www.producthunt.com/products/novu`) + `createdAt`(`2026-06-15T07:01:00Z`) 확보. 합성 완전 제거.
- **dcinside — detail body audit 결과 DETAIL_BODY_ALIVE(본문 230자 추출 가능)**이나, 정책상 **본문을 저장하지 않는다**(list community_signal만). 즉 기술적 본문부재가 아니라 정책 선택으로 미수집. `audit_dcinside_detail_body`.
- **EvidenceGate**: known synthetic/dead URL + local path를 거부하고 shape를 검사한다. **liveness/relevance 강제는 게이트가 아니라 fetcher가 수집 시점에** 수행한다(게이트는 shape+regression 가드로 범위를 정직히 한정). 신규 모듈 `evidence_gate.py`.

## Phase G-4 — dcinside detail body 정직 재확인 + culture/ph 실 url

- **dcinside detail body static audit = DETAIL_BODY_ALIVE(best_body_chars=230)**. 본문 추출이 기술적으로 가능하나 230자로 짧아, 본문 source로 승격하지 않고 **보수적으로 community preview signal 역할 유지**(full body·작성자 닉네임 PII 미수집). 즉 기술적 body 부재가 아니라 정책·역할 선택이며, 이를 degradation이 아닌 역할 정의로 명확화했다(02 참조).
- **culture_info — detail2 실 외부 전시 url + startDate**(data.go.kr period2→detail2). placeUrl 폴백 제거 유지(무관 venue url 둔갑 금지).
- **product_hunt — GraphQL 실 url + createdAt**(합성 name-slug fallback 거부 유지).
- 두 소스의 url/timestamp 복원력은 격리 dedup namespace proof로 source별 contract_pass=True 입증(eq=5/raw=5 각각, 05 참조).
