...
## 13. Phase D-P / E-0 Production Closure (2026-06-14)

**REDIS_URL 운영 리스크 방어 테스트화**: `test_pipeline_connectivity.py::test_redis_url_present_but_explicit_jsonl_queue_is_safe`가 `REDIS_URL` 설정 환경에서 `EventQueue(redis_url="", fallback_dir=tmp)`로 JSONL을 명시하면 `NotImplementedError` 없이 enqueue됨을 회귀로 고정(이전엔 문서 경고만).

**QualityPreGateResult 저장 스키마(E-0 pre-gate, `quality_pre_gate.py`)**: candidate별 `decision(pass/hold/reject)` + `reasons` + `normalized_published_at` + `canonical_url` + `evidence_ref` + `duplicate_key` + `boilerplate_risk` + `publication_policy`. 이는 EventSeed/큐 계약을 바꾸지 않는 **다운스트림 게이트 산출물**(아직 raw_events 아님).

**structured_signal 집계 주의(정직 고지)**: `summarize_expansion`의 `body_state_distribution` 합은 candidate_total과 다를 수 있다 — numeric/market 소스가 기사 0분해여도 유효 payload면 source-level `structured_signal`(numeric_exempt) +1로 세기 때문(실측 +3: coinbase/alpha_vantage 등). candidate-level과 source-level signal은 의도적으로 분리 집계한다.
