...
4. **R-IdentityHumanLabeling**(MEDIUM, **OPEN·protocol 부분진전** ADR#43+#44+#45) — 워크시트→gold 승격 + reviewer agreement protocol. **ADR#44**: GoldPair provenance·self-label 금지·gold-only metric·readiness. **ADR#45**: `resolve_gold_from_reviewers`(다중 reviewer 합의=gold·**conflict 자동 gold 금지**·LLM adjudicator 차단)·sampling bucket·통계적 표본 floor 추정(189/381). **미해소**: 실 human-reviewed gold 0·reviewer agreement 실측 0(→R-ReviewerAgreement)·sampling 대표성(→R-GoldSamplingBias)·SLA·주기 루프. **protocol 코드 ≠ 실 gold/agreement** — 완전종결 금지.
   - **R-ReviewerAgreement**(MEDIUM, **신규 OPEN** ADR#45) — 다중 reviewer 합의 실측·kappa·운영 절차 0(protocol 코드만).
   - **R-GoldSamplingBias**(MEDIUM, **신규 OPEN** ADR#45) — gold sampling 대표성·hard-negative/KO oversampling 실데이터 0(bucket 코드만).
