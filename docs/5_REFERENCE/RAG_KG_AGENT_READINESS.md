...
3. **R-IdentityEvalDataset**(MEDIUM, **OPEN·harness 부분 진전** ADR#42+#43) — adjudication status 가 self-labeled → 자기 precision 측정 불가. **ADR#43**: eval harness(`identity_eval_dataset.py`·fixtures·`evaluate_adjudicator`) 구축·현재 adjudicator precision **0.57**·FPR 0.2·KO 0.67 정직 측정(merge gate 미달·auto-merge OFF). **미해소**: live-derived+human-labeled gold set·규모·한국어 실 캘리브레이션·MERGE_GATE 런타임 배선. **실 병합 허용 판단의 선결**.
4. **R-IdentityHumanLabeling**(MEDIUM, **신규 OPEN** ADR#43) — export 워크시트(`export_identity_eval_pairs`)→gold 승격에 human labeling workflow 부재 → dead-data 한 단계 미룸. workflow + labeled gold 누적 필요.
