...
4. **R-IdentityHumanLabeling**(MEDIUM, **OPEN·workflow 부분종결** ADR#43+#44) — export 워크시트→gold 승격 workflow. **ADR#44**: `identity_human_labeling.py`(GoldPair provenance·`promote_worksheet_to_gold` self-label 금지·gold-only metric·`evaluate_gold_merge_readiness` live gold만·auto-merge OFF) + sample gold(시연). **미해소**: 실 human-reviewed production gold 0·SLA·reviewer agreement·주기 루프. **workflow 코드 ≠ 실 gold** — 완전종결 금지.
