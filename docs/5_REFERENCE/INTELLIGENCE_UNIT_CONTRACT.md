...
- **semantic identity status**: cross-batch 동일성은 3단계 — ① 확정 anchor 병합(`event_identity_map`, ADR#40)
  ② 결정론 fingerprint 후보 LINK(`event_identity_candidate`→`event_links possible`, ADR#41, **병합 아님**)
  ③ semantic 실 병합(adjudicator, **미구현** — R-SemanticIdentityAdjudicator).
- **possible-link substrate**: `event_links(possible, reason='semantic_cross_batch_candidate')` 는 미래
  adjudicator 가 confirmed/rejected/merged 로 재판정할 **가역 입력**이다. 현재 소비처 0(정직) — adjudicator
  구현 전까지 substrate 보존일 뿐 "중복 해결" 아님.
