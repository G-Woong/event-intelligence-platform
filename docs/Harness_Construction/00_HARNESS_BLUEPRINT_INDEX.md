...
> 작성: 2026-06-19 / 상태: **구현 완료 · live 운영 중** (이 폴더 `00~05` = as-built 설계 참조. 코드 경로 hard-pin 은 `05`뿐(`harness_doctor.py:71`·`README.md:100`이 settings/hook 복구원으로 인용), `04`는 `RISK_CLOSED`가 인용, `00~03`은 설계 참조(코드 비참조). turn-closeout SKILL 은 섹션번호(`01 §3`·`03 §3`)로만 인용. 일회성 실행 스펙 `06`·`07`은 `docs/3_ARCHIVE/2026-06_harness_design/`로 이관됨)
...
> 배선(settings.json·훅 스크립트·스킬)의 구체 명세 = `05`. (구조 리팩토링·마이그레이션 순서 `06`·통폐합 스펙 `07`은 실행 완료 → `3_ARCHIVE/2026-06_harness_design/`.)
...
**설계도:** 이 폴더 `docs/Harness_Construction/00~05`(live 참조) + 루트 `README.md`. (실행 스펙 `06`·`07`은 archive.)
...
- **의사결정자:** `00`(이 파일) → `01` → `03`
- **구현자/유지보수:** `00` → `05`(배선) → `01·02·04`(산출물 명세). (구현 완료; 마이그레이션 순서 `06`은 archive)
