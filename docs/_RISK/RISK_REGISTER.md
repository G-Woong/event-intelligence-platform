...
- 잔여(open, LOW): **실 fetch APPEND 미관측**(같은 사건 재출현 필요; genesis 자연어 화면 도달은 ADR#31 로 입증) · **source-type fidelity**: 타입 라우팅·보존 결정론 입증(ADR#32) 단 **실 cross-source 비뉴스 Event 미관측**(단일소스 probe=싱글톤) · 주기 auto-trigger 미배선 · 운영 DB(event_intel) 0006 미배포(검증은 event_intel_test).
...
- 관계: R-EventTimelineS2Hardening(④ 운영 가동 잔여)·R-EventModelMigration(synthetic records 기준)·R-EventTimelineRenderHardening(**CLOSED — ADR#31 genesis 렌더**)·R-SourceTypeFidelityGate(비뉴스 발행 gate)·R-Integration(경로 A 실데이터)을 **제품 수준에서 통합 추적**(중복 등재 아님 — 분산된 잔여를 단일 closure 로 묶음).

### R-SourceTypeFidelityGate · 비뉴스 발행 차단+authority+held-dedup+fail-closed+강신호 core-policy, 약신호 weak-primary·fragility 잔여  — Severity: LOW (부분종결 2026-06-23 · ADR#33/#34/#35/#36)
- Area: pipeline / source-type fidelity / 발행 안전 / 투자조언 경계
- Description: `event_resolver.resolve_routing` 는 **signal 강도(duplicate/possible)+clique** 로만 CREATE/APPEND/HOLD 를 정하고 **source_type/record_type 을 보지 않는다(type-blind — 파라미터조차 없음)**. 설계(`03_SOURCE_STATUS:30-31,40-42`)는 community/search→never_direct_publish, structured/market→`signal_only_not_article_card` 로 규정하나, **그 강제는 경로 A(event_cards) publication_policy 에만 있고 경로 B(Event 타임라인/event_resolver)엔 없다**. 결과로 비뉴스 cross-source 클러스터가 **발행 Event 를 생성**한다(ADR#32 결정론 재현): **S5** pure-community 강신호(동일 URL)→CREATE(evidence=[community]) · **S6** structured 2종 동일 signal-key→CREATE(evidence=[signal,signal]; **시장 신호가 Event 화 — 투자조언 금지 경로**) · **S7** pure-community 약신호(유사 제목)→CREATE 저신뢰. 즉 설계 의도와 구현 divergence — **강+약신호, community+structured 모두**.
- 입증/완화(현재): ① **structured signal-only 는 "설계 게이트"가 아니라 "비겹침 부수효과"** — 단일 스냅샷(S2/S3)은 0 Event 지만 동일 signal-key 충돌(S6)이면 발행. ② community+news 약신호는 news primary CREATE + community HELD(S4; R-FalseMerge 보호 작동 — 이 경로는 정상). ③ EVENT_RESOLUTION_ENABLED 기본 off + 현재 검증된 실 운영은 뉴스 위주(ADR#29) → 비뉴스 발행은 **잠재**(community/structured 소스 활성 + cross-source 겹침 필요). ④ evidence source_type 은 끝까지 보존(fidelity 자체는 유지). 완화 가정(rarity)은 약함: HN+Reddit 동일 URL/유사 제목 겹침은 드물지 않음(adversarial).
- ✅ **부분종결(ADR#33 source-type publish gate, 2026-06-23):** `resolve_routing` 에 `member_source_types` + 신규 `ACTION_WITHHELD` — 미매핑 CREATE 인데 publishable 멤버(official/article)가 0이면 **WITHHELD(미영속·미노출·멱등; DB 미접근·commit 0)**. S5/S6/S7 모두 created=0·withheld=1 재현(결정론+단위 12+ingest 3+live-PG 2). **pure community/search/structured 단독 cross-source 직접 발행 차단 = 핵심 안전 종결**(투자조언 경계 S6 포함). 매핑된 event APPEND(community corroborator)는 미적용. publishable 값 계약 drift 테스트로 잠금(P2-2). withheld_source_type 집계로 가시화(silent drop 아님).
- ✅ **부분종결 추가(ADR#34 primary-authority, 2026-06-23):** `candidate_from_cluster` 가 mixed cluster 의 Event 대표(title/도메인/관측시각/delta_summary kind/primary evidence)를 **최고 authority source**(official5>article4>signal3>search2>community1>미지0)로 선정 — **community/market 이 Event 대표가 되는 것 차단**(title 해소). tie=입력순(결정적·단일타입 회귀 0). 단위 4(official>community·news>market·news>community 약신호·news+news tie)+live-PG 1.
- ✅ **부분종결 추가(ADR#35 held-dedup + fail-closed, 2026-06-23):** ① **held-dedup**: `ResolvedCandidate.primary_member_key` + `apply_routing` held 루프가 대표 멤버 제외 → 대표 record 의 held degenerate **DB 이중등장 제거**(키 정확 일치만·corroborator 유지; 단위 2+영속층 1). ② **fail-closed**: `resolve_routing` gate 가 publishable 0이면(빈/미지/누락 포함) WITHHELD — **source_type 누락의 조용한 발행 우회 차단**(`_cand` 합성 helper 도 명시 source_type; 미지 record_type→WITHHELD 잠금).
- ✅ **부분종결 추가(ADR#36 weak-primary core-policy, 2026-06-23):** 강신호(duplicate) cluster 는 **강신호 core**(distinct−weak_only)에서만 primary 선정 + gate 입력(`core_source_types`)을 core source_type 으로 → **weak_only publishable 로 발행 안 함(WITHHELD)·weak_only 는 대표 불가**(후보 A core 우선+C WITHHELD; B 저신뢰발행 기각). 약신호(possible_duplicate)는 전체 동등 저신뢰(ADR#29 뉴스 흐름 보존). 단위 3(강 community/market core+weak→WITHHELD·강 news core+weak community→news 대표)+live-PG 1. **강신호 weak-primary 보수적 해소·R-FalseMerge 강화**.
- 잔여(open, LOW): ① **약신호 cluster weak-primary 미해소**(adversarial P2) — possible_duplicate 는 core=전체라 authority 높은 weak 멤버(약신호 title-link official)가 여전히 대표·발행 가능(강신호만 닫음; 약신호는 ADR#29 흐름 보존 명목). ② **members[0]→core→gate fragility**(P2) — 두-강성분 약신호 브릿지 cluster 의 core(=발행 가부)가 입력순서 의존 → 발행/미발행 뒤집힐 수 있음(R-FalseMerge 약신호 split 동일 영역; 입력순서-불변 회귀 미작성). ③ 비-primary weak corroborator evidence+held 이중등장(기존)·delta kind primary-only·커스텀 candidate_for fail-open(가드 밖).
- Closure(완전 종결): ✅ gate(#33)+authority(#34)+held-dedup·fail-closed(#35)+강신호 core-policy(#36). **남은 완전종결 조건**: 약신호 cluster weak-primary 결정/해소(WITHHELD/저신뢰표시/core 우선) + 두-강성분 core 입력순서-불변(R-FalseMerge 약신호 split 와 함께). severity **LOW**. **완전 종결(closed) 아님** — adversarial **OVERCLAIM**: 약신호 weak-primary 미해소 + fragility gate 전이가 closure 차단.
...
- Remaining gap(정직): ① **held event 중복/승격 정책** — held member 가 나중에 강신호로 자기 resolution 시 중복 Event 가능(S2e+ 재평가). **+ authority weak-primary 교차(ADR#35):** primary-authority 가 weak_only(약신호) publishable 멤버를 Event 대표로 세우면 그 약신호 결합이 검증 없이 Event 얼굴이 됨(held-dedup 이 DB 이중등장은 막으나 시맨틱 정당화 미완) — R-SourceTypeFidelityGate 잔여와 동일 영역. **(ADR#36 부분 해소: 강신호 cluster 는 core-policy 로 닫음·약신호 cluster weak-primary 는 잔여.)** ② **약신호 cluster_id split(신규, adversarial)** — 약신호(meta:/search: 키) 클러스터의 cluster_id 는 `members[0]` 최소인덱스 키에 고정되어, written_records 입력 순서가 배치 간 바뀌면 cluster_id 변동 → **같은 사건 재-CREATE(split)** 가능. 현재 강신호만 테스트/활성(EVENT_RESOLUTION_ENABLED off 기본)이라 무영향이나, **약신호 Event 활성화 전 cluster_id 입력순서-불변 회귀 필수**. **+ ADR#36 core→gate 전이:** 두-강성분 약신호 브릿지 cluster 의 core(=발행 가부)도 members[0] 입력순서 의존 → 발행/미발행 뒤집힘(R-SourceTypeFidelityGate 잔여②). ③ apply_routing rollback *경로 트리거*는 비결정(결과 불변식 orphan 0 은 live-PG 입증; 로직 unit 커버); 배치 단일 tx 화 시 `begin_nested()` SAVEPOINT 격리는 미래 하드닝. signal_strength 게이팅/heat 사용 시 대표값·`max` 편향 재평가.
