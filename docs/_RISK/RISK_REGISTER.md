...
- Closure: **live-derived + human-labeled** identity pair set(통계 규모) + production precision/recall 측정 + 한국어 실 캘리브레이션 + MERGE_GATE 런타임 배선 + 표본 floor 신뢰구간 재유도. **harness 구축 ≠ gold set 충족** — 완전종결 금지(fixture/sample 만으로 닫으면 OVERCLAIM).

### R-IdentityHumanLabeling · 워크시트→gold 승격 workflow(코드 DONE·실 production gold 0)  — Severity: MEDIUM (신규 2026-06-24 · ADR#43 — adversarial 권고; ADR#44 workflow 부분종결)
...
- Current mitigation: 승격 경로(export→promote→write→load→evaluate)가 코드·테스트로 동작 입증·결정론·Event 불변. raw body/PII/self-label/보조키 누출 구조적 차단.
- Remaining gap(정직, adversarial): ① **실 human-reviewed production gold 0**(샘플은 hand-authored 시연 — 실 reviewer 검수·통계 규모 0) ② human labeling 담당/절차/SLA 0 ③ reviewer agreement(다중 검수 합의) 0 ④ 주기적 export→label→eval 루프 미배선 ⑤ 표본 floor(live 200/KO 50)=통계 근거 없는 **draft placeholder**(LOW; 신뢰구간 재유도 필요).
- Closure: **실 human-reviewed live-derived gold 누적**(통계 규모) + human labeling 운영 절차/SLA + reviewer agreement + 표본 floor 신뢰구간 재유도 + export→label→`evaluate_adjudicator_on_gold` 주기 루프. **workflow 코드 ≠ 실 gold** — 코드만으로 완전종결 금지(OVERCLAIM).
