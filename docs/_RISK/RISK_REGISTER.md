...
### R-RealSourceLoopUnproven · 실제 웹 소스 데이터가 Event 타임라인(경로 B) end-to-end 를 흐른 적 0회  — Severity: MEDIUM (신규 2026-06-23 · REAL_SOURCE_LOOP_AUDIT 3-agent)
- Area: product / pipeline / Event 타임라인 / 검증 · **제품 핵심 가치(실 웹 인텔리전스) 미입증**
- Description: **전수 감사(2026-06-23, 3-agent)** 결과 — 두 경로를 절대 혼동 말 것. **경로 A**(수집→raw_events; raw_events→event_cards)는 **실데이터 PROVEN**: live probe(`api_live_probe_round_a.jsonl`)·최신 production-validation(`ingestion/outputs/_prod_orch.log` 47소스·653 extract→**608 raw_events 적재**[37 dup·critical_alert 1], `raw_events_mirror.jsonl` 누적 897건)·ap_news 100건→event_cards(R-Integration 2f, 단 **전량 status=hold→사용자 노출 0**=운반 능력 입증). **경로 B**(수집→cross_source_dedup→`event_ingest_pipeline`→`event_resolution_pipeline`→`event_timeline_service`→`/api/events/timeline`→`/events/timeline`)는 **코드 end-to-end 배선 완료이나 실 웹 데이터가 흐른 적 0회**. 경로 B 의 모든 입증은 ① in-memory fake session(`test_event_ingest_pipeline.py`), ② 하드코딩 synthetic record on live-PG test DB(`test_event_resolution_live_pg.py` `_rec(...)`), ③ `example.com` 합성 seed(D-2c `seed_event_timeline.py`)뿐. 최신 production-validation 로그는 `raw_events_target: mirror` + **event_resolution sink 미주입**(`event_resolution` 출력 라인 부재). `EVENT_RESOLUTION_ENABLED` off-by-default 이며 실 수집과 결선 가동된 적 없음.
- 완화(유지): 경로 B 코드 end-to-end **배선 완료(능력 확보)** · 각 단계 synthetic 입증(live-PG 실 Postgres 에서 CREATE/APPEND/HOLD·멱등·2-세션 동시성·FK RESTRICT) · 경로 A 실데이터 입증 · **flag off-by-default**(검증 전 자동 축적=쓰레기 데이터 차단). **D-2c synthetic seed 는 경로 B 의 단계 1~5(fetch→records→dedup→candidate→resolver)를 우회**하고 `event_timeline_service` 직접 영속 → 화면 렌더 *능력* 만 입증(실 루프 아님).
- 잔여(open): 실 keyless 소스 → 경로 B → 화면까지 **실데이터 1회 입증 0** · 주기 auto-trigger 미배선(`event_ingest_pipeline.py` 잔여 주석) · **delta_summary 실경로=디버그 라벨**(`event_ingest_pipeline.py:174` `"{confidence}:{reason}"` — R-EventTimelineRenderHardening② / synthetic seed 가 자연어로 *가림*) · **source-type fidelity**(news/official/numeric/catalog/community 성격이 Event evidence·timeline 에 보존되는지) 실데이터 미검증.
- Closure(전 조건 충족 시 종결): ① keyless/safe 소스 2~5개(타입 다양) 선정·expected behavior 문서화 → ② 실 수집(rate/robots/ToS 준수) → ③ records→cross_source_dedup→resolver→Event CREATE/APPEND/HOLD **실데이터 입증** → ④ `/api/events/timeline` 실데이터 non-empty → ⑤ `/events/timeline` 실데이터 렌더(스크린샷) → ⑥ source-type fidelity 보존 → ⑦ delta_summary 사용자 가독(또는 R-EventTimelineRenderHardening② 별도 종결). 실패 소스는 원인별로 기록(숨기지 않음). severity MEDIUM(배선·경로 A·synthetic 입증은 존재하나, 제품 북극성=실 웹 인텔리전스가 경로 B 에서 0회 입증).
...
### R-EventTimelineRenderHardening · Event 타임라인 렌더 잔여 보강 — ③+① 종결, ② 잔여  — Severity: LOW→LOW (부분종결 2026-06-23 · ADR#26)
- Area: frontend / web / ux / 안전
- Description(잔여): ② **delta_summary 본문 품질** — 현재 결선(`event_ingest_pipeline`)이 `"{confidence}:{reason}"` 디버그 라벨을 delta_summary 에 넣어, 데이터가 있어도 타임라인 본문이 사용자용 자연어 서술이 아님(상류 생성 책임). [① 상세 에러표현·③ source_refs wire 노출은 ADR#26 에서 종결 — 아래.]
- ✅ 종결(ADR#26, 2026-06-23): ③ **내부 식별자 wire 노출** → `PublicEvent`(primary_entity_ids=entities FK·snapshot_card_id=event_cards FK 제외)·`PublicEventUpdate`(source_refs 제외)·`PublicEventTimelineResponse` 신규로 공개 응답에서 **구조적 제외**(목록·단건 양쪽; frontend 타입도 제거; 테스트 `assert` source_refs·primary_entity_ids·snapshot_card_id 미노출). heat=제품 신호라 유지. ① **상세 비-404 에러표현** → 목록·상세 raw 없이 일반 안내 + **전역 `error.tsx` 도 raw message 미렌더**(render 단계 throw 까지 차단) → backend 장애 시 raw 내부 메시지 노출 전역 차단.
- 완화(유지): evidence url 안전 게이트(`isSafeHttpUrl` http/https + `rel="noopener noreferrer nofollow"`), allowlist 6키만 렌더(전문/PII 미노출).
- 잔여(open): ② delta_summary 자연어 서술 결선(상류 `event_ingest_pipeline:174` = `"{confidence}:{reason}"` 디버그 라벨). **D-2c synthetic seed 는 자연어 delta_summary 를 직접 써넣어 *데모에서만* 가독성을 보이나, 실 수집 경로는 여전히 디버그 라벨이다(seed 가 실 품질 문제를 *가림* — R-RealSourceLoopUnproven 과 연동).** deterministic template(source count/type/relation/confidence/domains 기반 자연어화)을 LLM 보다 먼저 검토.
- Closure: **실 수집 경로**의 delta_summary 가 사용자용 자연어 서술로 채워지면 완전 종결(synthetic seed 가독성은 종결 근거 아님). severity LOW(핵심 안전·wire 노출·에러표현은 종결; 잔여는 본문 가독성=상류 책임).
