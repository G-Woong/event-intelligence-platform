...
### R-IdentityHumanLabeling · 워크시트→gold 승격 workflow + reviewer agreement protocol(코드 DONE·실 production gold/agreement 0)  — Severity: MEDIUM (신규 2026-06-24 · ADR#43 — adversarial 권고; ADR#44 workflow 부분종결; ADR#45 reviewer agreement protocol 부분진전)
...
- Current mitigation: 승격 경로(export→promote→write→load→evaluate) + reviewer agreement protocol(합의/conflict/adjudication resolution) 코드·테스트로 동작 입증·결정론·Event 불변. raw body/PII/self-label/model-label/보조키 누출 구조적 차단.
...
### R-ReviewerAgreement · 다중 reviewer 합의 실측·운영 절차 부재(protocol 코드만)  — Severity: MEDIUM (신규 2026-06-24 · ADR#45 — adversarial 권고)
...
- Current mitigation: percent-agreement 산출·conflict no-auto-gold·만장일치만 agreed(2:1 다수결→conflict)·model/self/LLM adjudicator label 거부(fail-loud) — 코드·테스트로 안전 잠금.
- Remaining gap: ① 실 다중 reviewer 합의 데이터 0 ② Cohen kappa(규모 확보 후) 미산출 ③ reviewer 운영 절차/SLA/충원 0 ④ conflict adjudication 담당(사람 lead) 미배치.
- Closure: 실 다중 reviewer 합의 데이터(통계 규모) + agreement rate/kappa 실측 + reviewer 운영 절차 + adjudication 담당. **합의 protocol 코드 ≠ 합의 실측** — 코드만으로 종결 금지.

### R-GoldSamplingBias · gold sampling 대표성·oversampling 실데이터 부재(bucket 코드만)  — Severity: MEDIUM (신규 2026-06-24 · ADR#45 — adversarial 권고)
...
- Current mitigation: bucket 결정론 배정·미분류(other) 경고·draft min target(20)·insufficient 전용 bucket 라우팅(조용한 누락 차단) — 코드·테스트로 가시화.
- Remaining gap: ① 실 sampling 데이터 0 ② bucket 별 충원·oversampling 미실행 ③ min target 통계 근거 미확정(placeholder) ④ 대표성 검증 절차 0.
- Closure: 실 live-derived sampling + bucket 별 충원(hard-negative/KO/mixed oversample) + min target 통계 근거 + 대표성 검증. **bucket 코드 ≠ 대표 표본** — 코드만으로 종결 금지.
