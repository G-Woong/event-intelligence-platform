...
### R-LiveIdentityBacklog · live-derived identity candidate/adjudication 백로그 자체가 0(packet pilot 이 읽을 실 운영 후보 부재)  — Severity: MEDIUM (신규 2026-06-24 · ADR#47 — 실 probe 확정 blocker; ADR#48 stage③ 배선+migration readiness 부분진전)
...
- Current mitigation: 도구가 backlog/exclusion report 로 0 의 원인을 수치화(조용한 "후보 없음" 금지)·read-only(자동 병합/write 0)·stage③ 배선(flag·shadow·멱등)·migration readiness probe·실 파이프라인 유래 ingest→③→packet E2E(live-PG)로 경로·배선 입증.
- Remaining gap: ① 운영 DB(event_intel) alembic 0003→0009 마이그레이션·배포(배선 밖·배포 행위) ② 단계 ③ 주기 job 배선 — **no-cluster 배치는 ③ 미실행**(`if not clusters: return`)이라 이전 배치 미판정 link backfill 누락(주기 trigger 필요) ③ **O(N) 전수 재판정 비용** — 매 배치 모든 possible-link 재스캔(현 1행 무해·운영 볼륨 시 incremental adjudication[미판정 link 만]/배치 상한 필요·안전 아님) ④ 실 cross-source 후보 누적(news/official 충분 볼륨·실 fetch) ⑤ KO/hard-negative/mixed live bucket 충원.
- Closure: 운영 DB 마이그레이션·배포 + 단계 ③ 주기 운영 배선(no-cluster backfill·incremental) + 실 live-derived candidate/adjudication 백로그 누적(통계 규모) → packet pilot 이 synthetic/수동 주입 없이 live_selected>0 산출. **배선(능력) ≠ production(actuality)·도구 코드·1행 E2E ≠ 실 운영 백로그** — 코드만으로 종결 금지(OVERCLAIM).
