...
### R-LiveIdentityBacklog · live-derived identity candidate/adjudication 백로그 자체가 0(packet pilot 이 읽을 실 운영 후보 부재)  — Severity: MEDIUM (신규 2026-06-24 · ADR#47 — 실 probe 확정 blocker)
...
- Current mitigation: 도구가 backlog/exclusion report 로 0 의 원인을 수치화(조용한 "후보 없음" 금지)·read-only(자동 병합/write 0)·실 파이프라인 유래 1행 E2E(live-PG)로 경로 자체는 입증.
- Remaining gap: ① 운영 DB(event_intel) alembic 마이그레이션·배포 ② 단계 ③ adjudication 을 live 루프 또는 주기 job 으로 배선(결정론 유지) ③ 실 cross-source 후보 누적(news/official 충분 볼륨) ④ KO/hard-negative/mixed live bucket 충원.
- Closure: 운영 DB 마이그레이션 + 단계 ③ 운영 배선(결정론) + 실 live-derived candidate/adjudication 백로그 누적(통계 규모) → packet pilot 이 synthetic/수동 주입 없이 live_selected>0 산출. **도구 코드·1행 E2E ≠ 실 운영 백로그** — 코드만으로 종결 금지(OVERCLAIM).
