...
- 잔여(open): ② delta_summary 자연어 서술 결선(상류 `event_ingest_pipeline`/요약 생성 — D-2c+ 데이터 결선과 함께).
- Closure: delta_summary 가 사용자용 자연어 서술로 채워지면 완전 종결. severity LOW(핵심 안전·wire 노출·에러표현은 종결; 잔여는 본문 가독성=상류 책임).
