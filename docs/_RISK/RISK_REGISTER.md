...
### R-EventTimelineS2Hardening · S1 Event 토대의 S2 이전 확정 필요 항목  — Severity: LOW (신규 2026-06-22 · S2d 에서 ②③④ 처리 · S2e 에서 ① 정책 결정)
...
- DONE(S2d, 2026-06-22 — `event_timeline_service`): **②③④ 처리.** ② `_ensure_aware`(tz-naive→UTC)+`_coerce_uuid`(UUID/str 경계)를 CRUD 입력 전 적용. ③ `set_snapshot` 이 카드 탈취 거부 + **세팅 후 실제 영속값(card.event_id·event.snapshot_card_id)을 재조회**해 is_snapshot_bidirectional 검증(의도값끼리 비교하는 형식 단언이 아님 — 존재하지 않는 event 에 스냅샷 세팅도 차단). ④ `_sanitize_evidence`(allowlist 키 + **scalar 값만** — 중첩 dict/list 은닉 차단)+`_sanitize_source_refs`(짧은 식별자만) 로 **전문 본문·임의 PII 필드 영속 차단**(append_update 경로 + S2e 통합 파이프라인 통과 후에도 유지 입증). 회귀: test_event_timeline_service 27.
- 일부 DONE(S2e, 2026-06-22 — ADR#20): ① **app-layer 정책만 결정** — "app-layer hard-delete 미제공 + status 라이프사이클(events.status active→dormant→closed, event_links possible→confirmed/rejected/merged)". 삭제 메서드 부재(현재 0, 강제 유지)로 **app 경로 한정** 감사 이력 손실 트리거 0. **단 DB 레벨 미보호(adversarial B-2):** FK 가 전부 여전히 `ondelete="CASCADE"` → `DELETE FROM events` 한 번이 event_updates(감사 이력)를 cascade 삭제. "감사 보호"는 app 경로뿐이고 DB 레벨은 FK RESTRICT 전까지 미달성.
- 잔여(open): ① **DB 레벨 감사 보호 미달성** — FK RESTRICT 전환 migration(0006) + cluster_event_map/event_links per-table 삭제정책은 **live-PG 검증 가능 시점 이월**(mock 기반·DB fixture 부재라 migration 미실증). ④ 의 sanitize 는 **서비스 영속 경계**에서 강제되나, `delta_summary`/`canonical_title` 은 passthrough(sanitize 미적용) — 이를 채우는 상류 ingestion/요약 단계의 전문·PII 최소수집 책임은 S8/요약 단계 재검토(legal 잔여, candidate_for 미배선).
- Closure: ① FK RESTRICT migration 실 live-PG 검증(DB 레벨 감사 보호 달성) + ④ 상류 단계(delta_summary/canonical_title 포함) 전문/PII 가드 시 종결. (②③ 는 S2d 실검증 완료; ① 은 app 정책만 결정·DB 레벨 미달성.)
...
### R-FalseMerge · Union-Find transitive 오염이 영속 Event로 전파  — Severity: MEDIUM→LOW-MEDIUM→LOW (S2e 통합 로직 E2E 입증 2026-06-22; 미종결 — live-PG·held 승격 정책 잔여)
...
- Remaining gap(정직): **live-PG E2E 미실행** — in-memory fake session 은 실 Postgres migration/FK CASCADE/실 동시 세션을 대체하지 않는다(DB fixture 부재). 교차-tx 동시 CREATE 의 *실 세션* 멱등(현재 cluster_event_map.cluster_id PK + rollback 로직으로 방어, 단 2-세션 라이브 미입증) + FK 동작은 live-PG 이월. **apply_routing 트랜잭션 소유 계약(adversarial A-1):** 동시 CREATE 패배 rollback 은 전체 `session.rollback()`(SAVEPOINT 아님) — 호출자가 보존할 미커밋 외부 작업이 있는 tx 안에서 호출하면 안 됨(배치는 클러스터마다 자체 commit 이라 안전). 배치 단일 tx 화 시 `begin_nested()` SAVEPOINT 격리 필요 → live-PG 하드닝 이월. **held event 중복/승격 정책 미확정** — held member 가 나중에 강신호로 자기 resolution 시 중복 Event 가능(S2e+ 재평가). signal_strength 를 게이팅/heat 에 쓰면 단일 클러스터 대표값·`max` 낙관 편향 재평가 필요.
- Closure: 통합 E2E(transitive-only 자동승격 0)는 로직 입증 완료 → **live-PG 2-세션 동시 CREATE 멱등 실증 + FK 동작 검증 + held event 중복/승격 정책 확정 + event_links edge-level provenance(split 가역)** 시 완전 종결. (S2e 에서 LOW 로 하향했으나 live-PG·held 정책 잔여로 open 유지 — 사용자 기준7.)
