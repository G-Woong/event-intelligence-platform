...
### R-SourceTypeFidelityGate · resolver 가 source_type 무관(signal 강도) 라우팅 — community/search/structured 의 발행 규칙 미강제  — Severity: MEDIUM (신규 2026-06-23 · ADR#32 비뉴스 검증; 비뉴스 소스 운영 발행 전 해소 필요)
...
- 잔여(open): resolver/publish 경로에 **source-type publish gate** 부재 — community/search/structured 단독 cross-source 의 발행 차단(또는 HOLD 강등)·primary authority 우선순위(official>news>community) 미반영. 경로 A/B 발행 게이트 불일치(`03_SOURCE_STATUS:42` 정정 반영). delta_summary kind 라벨이 primary record_type 만 반영(혼합 클러스터 부정확, 경미).
- Closure: event_resolver 에 source-type publish gate(community/search/structured 단독 cross-source → HOLD/미발행) + primary authority 우선순위 도입(ADR 선행, rule 변경) 후 종결. severity **MEDIUM**(투자조언 경계 직결·강약신호 양 경로 재현·경로 A/B 불일치; 큰 rule 변경이라 ADR#32 이월. **비뉴스 소스를 운영에서 Event 발행하기 전 선해소 권고** — adversarial).
