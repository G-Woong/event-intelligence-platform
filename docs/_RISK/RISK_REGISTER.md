...
### R-SourceTypeFidelityGate · resolver source_type 라우팅 — pure 비뉴스 발행 차단(gate done), primary-authority 잔여  — Severity: MEDIUM→LOW (부분종결 2026-06-23 · ADR#33 publish gate)
...
- 잔여(open, LOW): **primary-authority 미구현**(adversarial P2-1) — gate 는 "발행 가부"만 막고 primary 선정(`cross_source_dedup` members[0], authority 무관)은 통제 안 함 → **유일 publishable 멤버가 weak_only(held)면 community-primary Event 발행**(title 이 community). official>news>community 우선순위 필요(R-FalseMerge held 정책과 동일 영역). + **fail-open 구조**(P3-2: 커스텀 `candidate_for` 가 source_type 미제공 시 gate 우회). + delta_summary kind 라벨 primary-only(경미).
- Closure(완전 종결): ✅ source-type publish gate(ADR#33 완료) + **primary authority 우선순위(official>news>community) 도입**(candidate primary 선정 — held-publishable/community-primary 잔여 해소) 후 종결. severity **LOW**(핵심 발행 차단·투자조언 경계 S6 차단 done; 잔여는 primary-authority 품질 — held-publishable cross-source 겹침은 드묾).
