...
### R-SourceTypeFidelityGate · resolver source_type — pure 비뉴스 발행 차단(gate)+대표 authority(primary), held 이중등장 잔여  — Severity: LOW (부분종결 2026-06-23 · ADR#33 gate + ADR#34 primary-authority)
...
- 잔여(open, LOW): ① **held-publishable DB 이중 등장**(adversarial P2-a) — 유일 publishable 이 weak_only(held)면 대표 evidence ↔ held degenerate event 로 동시 등장(title 은 authority-correct·held degenerate 는 매핑 게이트로 공개 차단이나 **데이터 정합 잔여**). ② **fail-open**(P3-2: 커스텀 `candidate_for` source_type 미제공 시 gate·authority 우회; 기본 매퍼는 안전). ③ delta_summary kind primary-only(corroborator 미반영, 경미).
- Closure(완전 종결): ✅ publish gate(ADR#33) + ✅ primary-authority(ADR#34) — **title 대표는 해소**. **남은 완전종결 조건**: held-publishable 멤버가 대표일 때 held degenerate 이중 등장 제거(resolver `held_members` 에서 primary 제외) + fail-open 가드. severity **LOW**. **완전 종결(closed) 아님** — adversarial: title 해소가 closure 의 "held-publishable 잔여 해소"를 부분 충족(데이터 정합 잔여).
