...
### R-CrossBatchEventIdentity · 같은 사건이 배치 경계에서 새 Event 로 분열(UNDER-merge)  — Severity: MEDIUM (신규 2026-06-24 · ADR#39 — R-FalseMerge 에서 분리; ADR#40 deterministic 층 부분종결)
...
- **⚠ 부분종결 — 닫힌 범위 vs 미해결(정직):**
  - **닫힌 범위:** deterministic **shared-anchor** cross-batch identity — publishable core 가 **동일 canonical_url/official_id** 를 재등장(syndicated wire: AP→여러 매체 재게재 등). live-PG 입증.
  - **미해결(여전히 OPEN):** **공유 strong anchor 없는** 같은-사건 분열 — 두 **다른** 기사(다른 URL)가 같은 사건을 다른 배치에 보도. semantic/entity identity(embedding/LLM/KG) 필요·**미구현(mock-default)**. Description 의 "새 corroborating 기사 추가" 다수가 여기 해당 → **종결 아님.**
  - **잔여 false-merge 주의(신규):** canonical_url 정규화가 fragment 제거 → 같은 URL=다른 사건(live-blog/재사용 URL)이 cross-batch 로 잘못 APPEND 가능(identity 층 신규 결함 아님 — 기존 record_key 한계의 cross-batch 확장; 모니터링).
- Closure: semantic/entity identity 층 ADR + 공유 anchor 없는 같은-사건 수렴 입증 + fragment 오APPEND 가드. semantic 잔여를 별도 RISK(예: R-SemanticEventIdentity)로 분리 시 **ADR 분리 논거 선행**(분리가 본 RISK 종결 구실이 되어선 안 됨). **RAG/KG 이전 필수 gate**(substrate 분열은 downstream 오염). severity MEDIUM(deterministic 층으로 dominant syndication 케이스는 닫힘).
