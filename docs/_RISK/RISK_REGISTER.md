...
- Remaining gap(정직): ① **held event 중복/승격 정책** — held member 가 나중에 강신호로 자기 resolution 시 중복 Event 가능(S2e+ 재평가). *(authority weak-primary 교차는 ADR#37 에서 해소 — 약신호 동질-publishable gate + 입력순서 불변 core; R-SourceTypeFidelityGate CLOSED.)* ② ~~약신호 cluster_id split~~ **해소(ADR#37):** `members = sorted(members, key=keys[m])`(인덱스→키 정렬)로 cluster_id/primary_record_key 입력순서 불변 + `primary_root`=최대 강성분(동률 publishable 우선)으로 core/발행 가부 입력순서 불변. 회귀 `test_weak_cluster_id_stable_across_input_order`·`test_order_invariant_*`. ③ apply_routing rollback *경로 트리거*는 비결정(결과 불변식 orphan 0 은 live-PG 입증; 로직 unit 커버); 배치 단일 tx 화 시 `begin_nested()` SAVEPOINT 격리는 미래 하드닝. signal_strength 게이팅/heat 사용 시 대표값·`max` 편향 재평가.
- Closure: live-PG 2-세션 동시 CREATE 멱등(orphan 0) + FK + wiring CREATE→APPEND **실증 완료** + 약신호 cluster_id/core 입력순서 불변(ADR#37) → **held event 중복/승격 정책 + event_links edge-level provenance** 만 남음(S2e+/S3). 핵심 false-merge(merge 방향) 방어는 live-PG 입증 — LOW 유지·held 승격 정책 open.
