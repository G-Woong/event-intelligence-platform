...
- 잔여(open, LOW): ⑦ **delta_summary 실경로=디버그 라벨**(`event_ingest_pipeline.py:174` `"{confidence}:{reason}"` — 본 검증은 전량 CREATE 라 update 미영속이라 화면 미표시였으나 코드상 APPEND 시 디버그 라벨; R-EventTimelineRenderHardening②) · **APPEND 경로 실데이터 미관측**(같은 사건 재출현 필요) · **source-type fidelity**: 뉴스만 Event 형성(official/community/market 은 singleton → Event 미형성, 타입 보존 미입증) · CREATE genesis evidence 미표시 gap · 주기 auto-trigger 미배선 · 운영 DB(event_intel) 0006 미배포(검증은 event_intel_test).
- Closure(완전 종결): ⑦ delta_summary 사용자 가독(R-EventTimelineRenderHardening② 종결) + APPEND 실데이터 관측(또는 genesis evidence 표시) + 비-뉴스 타입 Event 형성 1회 + 주기 auto-trigger. severity **LOW**(제품 북극성=실 웹 인텔리전스 경로 B 핵심 흐름은 실데이터로 입증; 잔여는 본문 가독성·커버리지·운영 가동).
...
- 잔여(open): ② delta_summary 자연어 서술 결선(상류 `event_ingest_pipeline:174` = `"{confidence}:{reason}"` 디버그 라벨). **D-2c synthetic seed 는 자연어 delta_summary 를 직접 써넣어 *데모에서만* 가독성을 보이나, 실 수집 경로는 여전히 디버그 라벨이다(seed 가 실 품질 문제를 *가림* — R-RealSourceLoopUnproven 과 연동).** **실 소스 검증(2026-06-23, ADR#29)에서 확인:** 실데이터 2 Event 는 전량 CREATE(genesis)라 update 미영속 → delta_summary 화면 미표시였고, 코드상 APPEND 시 디버그 라벨 영속. deterministic template(source count/type/relation/confidence/domains 기반 자연어화)을 LLM 보다 먼저 검토.
