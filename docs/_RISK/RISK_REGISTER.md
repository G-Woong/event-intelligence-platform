...
- Closure: embedding/LLM/KG adjudicator + 실 병합으로 **중복 Event count 감소 입증**(live-PG) + 언어별 fingerprint 캘리브레이션 + labeled 평가셋 precision 입증 + production gate(P/G/F·unsafe 차단). **deterministic shadow 만으로 종결 금지** — 실 병합·평가셋 없이는 "측정 불가 라벨". RAG/KG 이전 필수 gate 유지(실 병합 미입증).

### R-IdentityEvalDataset · semantic adjudication 라벨이 self-labeled 라 precision 측정 불가  — Severity: MEDIUM (신규 2026-06-24 · ADR#42 — adversarial 권고)
- Area: semantic identity 평가 / eval dataset / adjudicator precision-recall
- Description: ADR#42 `event_identity_adjudication` 의 status 는 **deterministic heuristic 의 출력**이지 ground truth 가 아니다 — self-labeled substrate 로는 **자기 자신의 precision/recall 을 측정할 수 없다**. 미래 embedding/LLM adjudicator(또는 임계 캘리브레이션)를 평가하려면 human-labeled 또는 외부 ground-truth identity pair set 이 필요하나 현재 0개. 이게 없으면 "eval substrate"는 "측정 불가능한 라벨을 정당화하는 label"에 그친다(adversarial).
- Current mitigation: shadow status 분포 report(`summarize_adjudication` by_status/by_language) — 분포 모니터링은 되나 정답 검증 아님.
- Remaining gap: ① labeled identity pair set 0개 ② shadow 라벨 vs ground truth precision/recall 미측정 ③ 한국어/영어 분리 성능 미보고.
- Closure: 최소 labeled identity pair set 구축 + shadow 라벨 대비 precision/recall 측정 + 언어별 분리 보고. **이 RISK 가 닫혀야 실 병합(R-SemanticIdentityAdjudicator) 허용 판단 가능**(평가 없는 production merge 금지).
