...
### R-SemanticIdentityAdjudicator · 공유 anchor 없는 같은-사건 실 병합 미구현(후보만 표면화)  — Severity: MEDIUM (신규 2026-06-24 · ADR#41 — R-CrossBatchEventIdentity 의 실 병합 잔여 분리)
...
- 후보 정책(ADR#41 §3): (A) deterministic 후보 substrate(이번 턴 DONE — LINK only) (B) future entity-slot/LLM hook contract(문서화 DONE — `INTELLIGENCE_UNIT_CONTRACT.md`) (C) **embedding/LLM adjudicator 즉시 도입 금지**(평가셋/cost/latency/citation/shadow 미비 → offline evaluator·shadow mode 로 먼저). production path 직접 도입 금지.
- Current mitigation: `event_links(possible, semantic_cross_batch_candidate)` 가역 raw substrate(append-only·possible→confirmed/rejected/merged). false-merge surface 0(LINK only).
- Remaining gap: ① possible 링크를 소비하는 실 adjudicator(embedding/LLM/KG) 0개 ② 실 병합으로 중복 Event count 감소 미입증 ③ fingerprint 한국어 토큰화 4-임계 캘리브레이션 부재(재현율 위험) ④ shadow/offline 평가셋·cost/latency/citation 기준 미비.
- Closure: offline evaluator + shadow-mode adjudicator(citation-grounded) + 실 병합으로 **중복 Event count 감소 입증**(live-PG) + 언어별 fingerprint 캘리브레이션 + production gate(P/G/F 경계·unsafe 차단). **이게 없으면 본 RISK 는 "죽은 LINK 를 정당화하는 라벨"** — 등록만으로 진전 주장 금지.
