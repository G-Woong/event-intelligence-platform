...
### R-IdentityEvalDataset · semantic adjudication 라벨이 self-labeled 라 precision 측정 불가  — Severity: MEDIUM (신규 2026-06-24 · ADR#42; ADR#43 harness 부분 진전)
...
- **⚠ 미해소(OPEN, adversarial):** ① fixture 는 **진단(stress) 세트** — production precision 대표 표본 아님(0.57≠운영 precision)·통계 규모 부족(22 pair) ② **live-derived + human-labeled** gold set 0개 ③ 워크시트→gold 승격에 **human labeling workflow 부재**(→R-IdentityHumanLabeling) ④ 한국어 캘리브레이션은 **측정만**(0.67 재확인)·실제 교정(stopword/어절 임계) **미이행**(gap 또 이월) ⑤ MERGE_GATE 는 현재 **장식**(merge 코드 미존재·런타임 배선 없음).
- Current mitigation: harness + 진단 fixture + status/언어 분포 report — 측정 도구는 작동하나 production gold 아님.
- Closure: **live-derived + human-labeled** identity pair set(통계 규모) + production precision/recall 측정 + 한국어 실 캘리브레이션 + MERGE_GATE 런타임 배선. **harness 구축 ≠ gold set 충족** — 완전종결 금지(fixture 만으로 닫으면 OVERCLAIM).

### R-IdentityHumanLabeling · 워크시트→gold 승격 workflow 부재(dead-data 한 단계 미룸)  — Severity: MEDIUM (신규 2026-06-24 · ADR#43 — adversarial 권고)
- Area: semantic identity 평가 / human labeling / gold set 승격
- Description: ADR#43 `export_identity_eval_pairs` 가 `event_identity_adjudication`(소비처 0이던)을 워크시트 JSONL(label='unlabeled')로 소비해 dead-data 를 줄였으나, **사람이 gold label 을 채워 `identity_eval_pairs.jsonl`(R-IdentityEvalDataset)로 승격하는 workflow(담당·SLA·CI 라운드트립)가 없다** → 워크시트가 또 다른 휘발성 산출물이 될 수 있다(dead-data 형태 변환). 즉 "소비처 추가로 dead-data 감소"는 human labeling 이 실재해야만 참.
- Current mitigation: export 워크시트는 raw body/PII 0·결정론·`load_eval_pairs` 라운드트립(language latin→en 정규화 후 gold 승격) 테스트로 승격 경로 자체는 동작 입증.
- Remaining gap: ① human labeling 담당/절차/SLA 0 ② 라벨링된 gold 누적 0 ③ 주기적 export→label→eval 루프 미배선.
- Closure: human labeling workflow 문서 + 최소 labeled gold 누적 + export→label→`evaluate_adjudicator` 주기 루프. 이게 없으면 워크시트는 dead-data.
