...
- DONE(held 승격 candidate B, ADR#38 2026-06-23 — 부분종결): held member 가 다른 cluster_id 로 강신호 재등장 시 중복 Event 방지. `cross_source_dedup.titles_similar`(약신호 결합과 동일 기준) + `event_timeline_service.find_held_parents`(degenerate held→possible→**매핑된** parent) + `resolve_and_apply_cluster(title_matcher)`: 재등장 cluster 제목 ↔ parent 제목 **titles_similar 일 때만** parent 로 라우팅(강신호 APPEND·약신호 HOLD)+cluster_id→parent 매핑(멱등), 불일치면 독립 CREATE(false-merge 방어). 인메모리 회귀 7(official same→APPEND·different→CREATE·no-lineage→CREATE·community/market match→parent 연결 corroborator APPEND·mismatch→gate WITHHELD[투자조언 경계·직접발행 차단 유지]).
- Remaining gap(정직): ① **held 승격 완전 종결 보류**(adversarial OVERCLAIM): (a) **live-PG 미실증** — find_held_parents 신규 SQL(aliased self-join+cluster_event_map subquery)이 live-PG held 승격 3 테스트로 미실행(Docker 미가동 SKIP; 프로젝트 규율상 fake 대체 불가) → live-PG green 전까지 완전 종결 금지. (b) **cross-batch event identity**(별개·더 큰 문제) — 매 배치 신규 corroborating 기사가 새 cluster_id→새 Event 분열·비-held 확정 멤버 다른 cluster_id 재등장은 held 가 아니라 fix 밖(semantic/엔티티 동일성 층 이월). ② ~~약신호 cluster_id split~~ **해소(ADR#37)**. ③ apply_routing rollback *경로 트리거*는 비결정(결과 불변식 orphan 0 은 live-PG 입증; 로직 unit 커버); 배치 단일 tx 화 시 `begin_nested()` SAVEPOINT 격리는 미래 하드닝.
- Closure: live-PG 동시 CREATE 멱등(orphan 0)+FK+wiring CREATE→APPEND+약신호 cluster_id/core 입력순서 불변(ADR#37)+**held member 재등장 승격(ADR#38 인메모리·gate-bypass 입증)** → **남은 완전종결 조건: (a) live-PG held 승격 3 테스트 green(Docker 가동) (b) cross-batch event identity 결정/이월**. severity **LOW**. **완전 종결 아님** — adversarial OVERCLAIM(live-PG 미실증·cross-batch 잔여).
