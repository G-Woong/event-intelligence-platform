...
- 관계: R-EventTimelineS2Hardening(④ 운영 가동 잔여)·R-EventModelMigration(synthetic records 기준)·R-EventTimelineRenderHardening(**CLOSED — ADR#31 genesis 렌더**)·R-SourceTypeFidelityGate(**CLOSED — ADR#37 약신호 정책·입력순서 불변**)·R-Integration(경로 A 실데이터)을 **제품 수준에서 통합 추적**(중복 등재 아님 — 분산된 잔여를 단일 closure 로 묶음).
...
### R-CrossBatchEventIdentity · 같은 사건이 배치 경계에서 새 Event 로 분열(UNDER-merge)  — Severity: MEDIUM (신규 2026-06-24 · ADR#39 — R-FalseMerge 에서 분리)
- Area: Event identity / lineage / 중복 Event / RAG·KG substrate 무결성
- Description: Event identity 가 `cluster_id = xcluster:{min(member record_keys)}`(`cross_source_dedup.py`)에 묶임. 같은 사건의 **새 corroborating 기사/공식문서가 다음 배치에 추가**되면 cluster 멤버십(최소 record_key)이 바뀌어 cluster_id 가 달라지고 → cluster_event_map 미매핑 → **새 Event CREATE → 같은 사건이 배치마다 분열**(timeline/RAG/KG/Entity graph/LLM routing 오염). R-FalseMerge(OVER-merge)의 **반대 실패모드**(UNDER-merge). ADR#38 held 승격은 held member(degenerate+possible)만 커버 — 비-held 확정 멤버(강신호 mapped core)의 cross-batch 재등장은 find_held_parents 무매칭 → fix 밖.
- 구분(왜 별도 RISK): OVER-merge 방어(병합 차단 게이트)와 UNDER-merge 방어(동일성 식별 층)는 종결 조건이 상호 배타(병합 막을수록 분열↑) → 동일 RISK 불가. adversarial JUSTIFIED(ADR#39: over/under 역방향).
- 후보 정책: (A) record_key→event_id 역인덱스(멤버가 이미 Event 에 속하면 CREATE 대신 APPEND) — syndicated wire 공유 기사에서 신규 false-merge 위험. (B) entity+time+domain semantic identity 층. (C) 별도 ADR 설계 후 구현. **현재 미구현** — 작은 deterministic guard 도 syndication false-merge 분석 선행 필요 → ADR 우선(hasty guard 금지).
- Remaining gap: cluster_id 가 멤버십 의존 → 배치 간 동일성 미보장. 실 cross-batch 분열 라이브 미관측(현 production-validation 단발 배치).
- Closure: Event identity 층 ADR(record_key/entity 기반) + live-PG cross-batch 테스트(배치1 {A,B}→E, 배치2 {A,B,C}→동일 E APPEND·분열 0) + syndication false-merge 회귀. **RAG/KG 이전 필수 gate**(substrate 분열은 downstream 전파 오염).

### R-SourceCatalogFidelity · catalog 메타데이터(domain) 가 official_record 로 발행  — Severity: MEDIUM (신규 2026-06-24 · ADR#39)
- Area: source-type fidelity / Event 발행 정직성 / catalog 누수
- Description: `run_production_orchestration._GROUP_TO_RECORD_TYPE` 가 `domain→official_record`. catalog 6종(aladin/tmdb/kofic/kopis/tour/igdb)이 전부 source_group `domain`(source_profiles.yaml) → eq_record record_type=official_record → `event_ingest_pipeline._RECORD_TYPE_TO_SOURCE_TYPE` 가 official source_type → **publishable "official" Event 로 발행 가능**(영화/도서/박스오피스 메타가 권위 사건으로 노출). source-type publish gate(ADR#33/#35)의 전제(official=권위 출처)를 상류에서 무너뜨림.
- 3중 drift: `_GROUP_TO_RECORD_TYPE`(domain→official_record, **live 할당**) ≠ `source_readiness_closure._GROUP_RECORD_TYPE`(domain→structured_signal, readiness 분석) ≠ `source_content_type`(catalog→catalog_metadata).
- 완화 요인(미발현): Event resolution flag 기본 off · 단일 catalog item=singleton drop(클러스터 미형성 시 미발행) · production-validation 에서 catalog cross-source Event 미관측. 단 catalog 2건 동질(official+official) cluster 면 발행 가능 — **누수 경로 실재**.
- 미수정 사유: `OFFICIAL_RECORD_ALIVE` 상태 taxonomy 에 박힌 의도된 설계(domain=official_record) — hasty-patch 가 status/test 회귀 위험 → **정책 ADR 필요**(catalog→structured_signal / 별도 catalog_metadata 비-publishable record_type / KG·entity enrichment 전용 중 택).
- Closure: catalog record_type 정책 ADR + catalog 소스→비-publishable 회귀 테스트(catalog 단독 cross-source 발행 0) + 3중 drift 정합 + live 검증(catalog 가 official Event 미발행). 관계: R-SourceTypeFidelityGate(CLOSED — gate 로직 정상, 본 RISK 는 **상류 record_type 라벨링 오류** = 별개 층).
