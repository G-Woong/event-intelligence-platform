...
| 12 | **12_EVENT_CLUSTERING_RANKING_AND_DEDUP** | 📘→🔲 | exact dedup+`cross_source_dedup` 토대만 실재; ②~⑤(Event append/timeline/heat/rank) 미구현. **임계경로(Event 라우팅)** |
...
S5  LLM Expansion Router (게이트 존재라 가장 저렴, S1 직후)  ── 파일 06+11+19
    ▼
S2  Event Resolution Engine + S3 domains/tags 2층  ── 파일 12+19  (R-FalseMerge: clique 게이트)
    ▼
