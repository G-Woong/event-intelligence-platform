...
| 12 | **12_EVENT_CLUSTERING_RANKING_AND_DEDUP** | 🟡 PARTIAL | dedup+clique 게이트(S2b) + event_resolver 라우팅(S2c) + CRUD 영속(S2d) 구현; 통합 E2E(S2e)·heat/rank(S2.5~) 미구현. **임계경로(Event 라우팅)** |
...
| 17 | **19_WEB_INTELLIGENCE_IMPLEMENTATION_SPEC** | 🟡 PARTIAL (S1+S2-core+S2d) | **S1 토대 + S2 Event Resolution 구현됨**(events/event_updates/event_cards.event_id+0004; cluster_event_map/event_links+0005; clique 게이트·event_resolver·event_timeline_service CRUD, 2026-06-22). 통합 E2E(S2e)·Expansion·Evidence Graph·Agent Debate(S5~S11) 미구현 |
...
S5  LLM Expansion Router (게이트 존재라 가장 저렴, S1 직후)  ── 파일 06+11+19
    ▼
S2  Event Resolution Engine [✓ S2-core a~c + S2d CRUD 영속 2026-06-22; S2e 통합 E2E 잔여] + S3 domains/tags 2층  ── 파일 12+19  (R-FalseMerge: clique 게이트)
    ▼
