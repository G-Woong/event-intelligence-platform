...
**잔여(주기 운영, ADR#50 갱신):** 주기 가동은 **새 scheduler 발명 불필요** — 기존 `workers/tools/run_recovery_scheduler`(`--interval-sec`/`--once`·docker `recovery-scheduler` 서비스) 식 관용구로 `python -m backend.app.tools.backfill_semantic_adjudications --limit N`(또는 `--once` cron) 을 구동(**단, 운영 DB 0003→0009 migration 후** — 미마이그레이션 DB 엔 테이블 부재로 크래시·게이트). **완화(ADR#50):** bounded run 의 cheap O(전체) 적재 → keyset(`after_link_id`·SQL `WHERE id>cursor·NOT IN·LIMIT`)으로 페이지만 로드(`full_scan` 플래그로 전수 경로 표면화)·교차 backfill 은 link_id PK upsert 로 중복행 0(데이터 안전·중복 work 는 단일 runner/disjoint cursor 권고·lock 과대주장 0·OS-병렬 race 미stress-test). **정직 경계:** cursor=`event_links.id` UUIDv4 byte 순서라 **시간순 아님**(재현 가능 페이지 경계)·진행 보장은 `only_unadjudicated`. **여전히 잔여:** 실제 docker 주기 서비스 배선(운영 DB 후·backfill 은 현재 자체 `--once` CLI 만·recovery-scheduler 미배선)·실 fetch volume·`full_scan` 전수 report 경로의 대형 백로그 비용. **실 병합(④)은 여전히 금지**(R-SemanticIdentityAdjudicator·MERGE_GATE 선결).

### RealSourceLoop 단계 상태 (ADR#50 갱신 — fetch→Intelligence Unit)
