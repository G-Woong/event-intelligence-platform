...
### 운영 DB 배포 runbook — stage③ live backlog 가동 (R-LiveIdentityBacklog · ADR#48 readiness + ADR#49)
...
8. **readiness 재실행**: `operational_db_readiness` → `on_head=True`·`ready_for_stage3=True`·`behind_count=0`.
...
4. **backfill dry-run**: `backfill_semantic_adjudications(session, dry_run=True)` → `pending_before`·예상 `processed` 규모 확인(영속 0).
5. **backfill limited run**: `backfill_semantic_adjudications(session, limit=N, dry_run=False)` → bounded chunk 로 누적 pending 따라잡기(`pending_after` 감소·Event 불변·`auto_merge_enabled=False`).
...
**잔여(주기 운영):** 주기 스케줄러(Celery beat/cron) 미배선·동시 backfill 직렬화 미명세(멱등이라 안전하나 중복 view-load — 절감 무효)·cheap O(all-links) 전수 스캔(대형 백로그 keyset 필요). **실 병합(④)은 여전히 금지**(R-SemanticIdentityAdjudicator·MERGE_GATE 선결).
