...
**ADR#86-ready spec — Federal Register window-honoring adapter + official×news role-bridge (이 턴 미배선·정밀 spec):**

- **adapter(`provider_query_adapters.py` additive)**: `FEDERAL_REGISTER_ADAPTER = ProviderQueryAdapter(provider_id="federal_register", query_capable=True, auth_required=False, required_env_vars=(), fetch_implemented=True, supports_topic_query=True, supports_time_window=True, supports_source_pair=False, max_records=25, rate_limit_policy_id="federal_register", host="www.federalregister.gov", host_gate_key="www.federalregister.gov", host_min_spacing_seconds=2)`. endpoint 는 registry 에 이미 존재(`https://www.federalregister.gov/api/v1/articles.json`·key-free).
- **url builder `_federal_register_url`**: params `conditions[term]=topic`·`conditions[publication_date][gte]=from_date`·`conditions[publication_date][lte]=to_date`(omit_date_window 시 제외)·`per_page=max_records`·`order=newest|relevance`(order knob)·`fields[]=title,html_url,publication_date,document_number`. **명시 범위 필터 → date-honoring 가능성 high(가설 검증 control)**.
- **parser `parse_federal_register_items`**: `data["results"][].{title, html_url, publication_date}` → `_rec(record_type="official_document_candidate", source_id="federal_register", title≤512, canonical=html_url, published=publication_date, body_state="present")`·role=official·본문 미저장. `_ADAPTERS`/`_URL_BUILDERS`/`_PARSERS` 등록.
- **rate_limit_policy.yaml**: `federal_register`(min_interval 2s·cooldown 60s·관대·official public API).
- **official×news role-bridge(별도 정책·guard 약화 금지)**: 현 `discover_overlap` 는 publishable×publishable(article)만 cross-source near-match. FR=official 이므로 **official(anchor)×article(news) date-anchored pairing** 을 허용하는 opt-in band 를 신설하되, community/market/catalog/search 의 anchor 금지는 **불변**. official document 는 event anchor·news 는 reaction/coverage 로 pairing(role tag 보존).
- **tests**: FR url(date params·order·omit)·no-auth(auth_required=False)·parser shape·enforce_window·official×news bridge band·role guard 비약화·secret 0.
- **risk(검증 대상)**: FR 반환물=규제문서(rules/notices)라 일반 news event 와 topical match 가 희소할 수 있음(date-honoring 은 검증되나 cross-source same-event 수율은 미지)·GDELT 는 rate/attribution 미해소로 ADR#86 범위 밖.
