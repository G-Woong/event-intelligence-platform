...
> **ADR#94 cross-ref (sprint/contract-only · runtime No-Go):** 실 payload 실행 직전/실행/실패분해/다음 반복을 하나의 operator sprint 로 묶는 7 모듈 — `5_REFERENCE/FIRST_REAL_PAYLOAD_EXECUTION_SPRINT.md`(real payload valid∧approved→validate→dry→live 1회·없으면 blocked+ready package·probe 미경유)·`5_REFERENCE/OPERATOR_CONFIRMED_READY_PACKAGE.md`(operator 외부확인용 package·real payload 아님·confirmed/approved False)·`5_REFERENCE/UNIFIED_LIVE_RESULT_CLOSURE.md`(taxonomy+overlap+breadth+provider+freeze 1-closure·truth/gold 아님)·`5_REFERENCE/FIRST_LIVE_FREEZE_R1_DRY_RUN_HARNESS.md`(합성 freeze 후보로 freeze→R1 경로 검증·production gold 0)·`5_REFERENCE/AI_REPLIES_GUARD_AUDIT.md`(ungated mock 엔드포인트 정적 audit·미수정·미import)·`5_REFERENCE/PUBLIC_RUNTIME_KILL_SWITCH_MAP.md`(8 public runtime 전부 disabled·override 는 R1∧R2+ADR+test)·`5_REFERENCE/SOURCE_GRAPH_TIMESERIES_INSIGHT_CONTRACT.md`(15 component candidate-only·official/news 만 anchor·MERGE_GATE 전 truth 아님). frontier parity 104→111·**R1 = FAIL·R2~R7 = No-Go 불변.**

> **ADR#95 cross-ref (contract-only · runtime No-Go):** ADR#94 sprint 다음 단계 — 증거보조 real payload 생산·operator 검증·real payload 템플릿 하드닝·첫 후보 evidence binder·reviewer packet dry-run·ai_replies 10-gate 게이트 설계·source-graph→Hot Post 통합 맵을 7 순수 contract 모듈로 묶음(network/LLM/embedding/DB/file-write/secret/sending 0). frontier +7 → 118(dict==pydantic==TS==mjs 118). 실 operator payload 미제공(`inputs/operator_events/` ABSENT)→실 live 미실행·production_gold 0·R1 gap 200·R2~R7 No-Go·public Hot Post/comment runtime 비활성.
